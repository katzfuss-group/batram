...
from .data import MultiFidelityData, AugmentDataMF
...
def kernel_fun(X1_now, X1_preb, theta_0, theta_1, theta_0_preb, theta_1_preb, theta_ls, smooth, 
               sigma, nugget_mean, X2_now = None, X2_preb = None):
    if X2_now is None:
        X2_now = X1_now
    if X2_preb is None:
        X2_preb = X1_preb
...
class TMKernelMF(torch.nn.Module):
    ...
    def _m_threshold(self, max_m: int, r: int, preb: bool, past_m: torch.Tensor | None = None) -> torch.Tensor:
    ...
    def _determine_m(self, max_m: int, r: int, preb: bool, past_m: torch.Tensor | None = None) -> torch.Tensor:
    ...
    def forward(self, data: AugmentedData, nug_means: torch.Tensor, r: int) -> KernelMFResult:
        ...
        g = k + eyes
        max_attempts = 10  # Maximum number of attempts with increasing diagonal
        eps = 1e-6        # Initial small diagonal value to add
        attempt = 0
...
class IntLogLikMF(torch.nn.Module):
    ...
    def precalc(self, kernel_result: KernelMFResult, response, 
                fidelity_sizes: torch.Tensor, r) -> _PreCalcLogLikMF:
        fs = fidelity_sizes
    ...
    def forward(self, data: AugmentedData, kernel_result: KernelMFResult, r: int) -> torch.Tensor:
        tmp_res = self.precalc(kernel_result, data.augmented_response[:,:,0], data.data.fidelity_sizes, r)
...
class MultiFidelityTM(torch.nn.Module):
    ...
    def forward(
        self, r: int, batch_idx: None | torch.Tensor = None, 
    ...
    def fit(
        ...
        if optimizer is None:
            ...
            optimizer = torch.optim.Adam(list(set(self.parameters()) - 
                                              {self.kernel._kernel.raw_lengthscale}), lr=init_lr)
        ...
        for r in range(self.R):
            ...
            scheduler = torch.optim.lr_scheduler.CosineAnnealingLR(optimizer, num_iter, eta_min=0.001)
                ...
                for j, idx in enumerate(idxes):
                    ...
                    with torch.autograd.set_detect_anomaly(True):
                        ...
                        def closure():
                            ...
                            # paste the final trained parameter here
                            for l in range(r):
                                if j != 0:
                                    # Nugget 1
                                    self.nugget.nugget_params.grad[l] = 0
                                    print(optimizer.state[self.nugget.nugget_params])
                                    optimizer.state[self.nugget.nugget_params]['exp_avg'][l] = 0
                                    optimizer.state[self.nugget.nugget_params]['exp_avg_sq'][l] = 0
                                    
                                    # Nugget 2
                                    self.nugget.nugget_params.grad[l+self.R] = 0
                                    optimizer.state[self.nugget.nugget_params]['exp_avg'][l+self.R] = 0
                                    optimizer.state[self.nugget.nugget_params]['exp_avg_sq'][l+self.R] = 0

                                    # Sigma 1
                                    self.kernel.sigma_params.grad[l] = 0
                                    optimizer.state[self.kernel.sigma_params]['exp_avg'][l] = 0
                                    optimizer.state[self.kernel.sigma_params]['exp_avg_sq'][l] = 0

                                    # Sigma 2
                                    self.kernel.sigma_params.grad[l+self.R] = 0
                                    optimizer.state[self.kernel.sigma_params]['exp_avg'][l+self.R] = 0
                                    optimizer.state[self.kernel.sigma_params]['exp_avg_sq'][l+self.R] = 0

                                    # Theta q 0
                                    self.kernel.theta_q.grad[l] = 0
                                    optimizer.state[self.kernel.theta_q]['exp_avg'][l] = 0
                                    optimizer.state[self.kernel.theta_q]['exp_avg_sq'][l] = 0

                                    # Theta q 1
                                    self.kernel.theta_q.grad[l+self.R] = 0
                                    optimizer.state[self.kernel.theta_q]['exp_avg'][l+self.R] = 0
                                    optimizer.state[self.kernel.theta_q]['exp_avg_sq'][l+self.R] = 0

                                    # Lengthscales
                                    self.kernel.lengthscale.grad[l] = 0
                                    optimizer.state[self.kernel.lengthscale]['exp_avg'][l] = 0
                                    optimizer.state[self.kernel.lengthscale]['exp_avg_sq'][l] = 0

                                    if l > 0:

                                        # theta q 0 preb
                                        self.kernel.theta_q.grad[l+2*self.R-1] = 0
                                        optimizer.state[self.kernel.theta_q]['exp_avg'][l+2*self.R-1] = 0
                                        optimizer.state[self.kernel.theta_q]["exp_avg_sq"][l+2*self.R-1] = 0


                                        # Theta q 1 preb
                                        self.kernel.theta_q.grad[l+3*self.R-2] = 0
                                        optimizer.state[self.kernel.theta_q]['exp_avg'][l+3*self.R-2] = 0
                                        optimizer.state[self.kernel.theta_q]["exp_avg_sq"][l+3*self.R-2] = 0
    ...
    def score(self, obs, r = 0, last_ind = None):
            ...
            for j in range(r, self.R):
                ...
                tmp_res = self.intloglik.precalc(kernel_results, augmented_data.response, fs, j)
                ...
                for i in range(fs[j]):
                    ...
                    if i == 0:
                        ...
                        cStar = self.kernel._kernel_fun(
                            XPred_now, XPred_prev, sigmas[i], nugget_mean[i], j, X_now, X_preb
                            ).squeeze(1)
    ...
    def cond_sample(self, obs, res, r = 0, last_ind = None, num_samples: int = 1):
