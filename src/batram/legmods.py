...
class SimpleTM(torch.nn.Module):
    ...
    def cond_sample(
        ...
        if obs is not None:
            raise NotImplementedError("Observations not implemented.")
    ...
    def score(self, obs, xFix=torch.tensor([]), indLast=None, mode: str = "score"):
        ...
        if indLast is None:
            indLast = N
        # loop over variables/locations
        scr = torch.zeros(nobs, N)
        for i in range(xFix.size(0), indLast):
            # predictive distribution for current sample
            if i == 0:
                cStar = torch.zeros((nobs, n))
                prVar = torch.zeros(nobs)
            else:
                ncol = min(i, m)
                X = data[:, NN[i, :ncol]]
            ...
            meanPred = yTilde[i, :].mul(cChol).sum(1)
            varPredNoNug = prVar - cChol.square().sum(1)
    ...
    def fit(
        ...
        scheduler
            Learning rate scheduler to use. If None, CosineAnnealingLR is used
            with default optimizer.
        stopper
            An early stopper. If None, no early stopping is used. Requires test
            data.
        method
            Method to use for fitting. 1 does not update process locations, 2
            does but does not reorder, 3 does both. When using a user provided
            optimizer, the updated parameters are defined there.
        silent
        ...
        if optimizer is None:
            ...
            if method == 1:
                optimizer = torch.optim.Adam(list(self.parameters())[0:1], lr=init_lr)
            else:
        ...
        next_reoder = 4
        termination: None | Terminanation = None
        for it in (tqdm_obj := tqdm(range(num_iter), disable=silent)):
            # create batches
            if batch_size == data_size:
            ...
            if stopper is not None:
                ...
                if stop:
                    ...
                    termination = Terminanation(
                        reason="early_stopping",
                        description=(
                            f"Stopped after {it} iterations. Best state encountered"
                            f" after {stopper.steps_best_state()} iterations. Minimal"
                            f" loss: {stopper.min_loss():.5f}."
                        ),
                    )

                    # report best test loss
                    with torch.no_grad():
                        test_loss_best = self(data=test_data).item()
                        print(f"Early stopping; best test loss: {test_loss_best:.5f}")
            ...
            # reorder method3
            reordered_pos = None
            if method == 3 and it == next_reoder:
                print("reordering")

                with torch.no_grad():
                    pos = self.data_mod.process_position()
                    reordered_pos = pos.numpy()
                self.data_mv = self.data_mv.reorder(pos)
                test_data = test_data.reorder(pos)

                next_reoder *= 2
                if stopper is not None:
                    stopper.reset()

        # handle case when no early stopping was encountered
        if termination is None:
            termination = Terminanation(
                reason="max_iter",
                description=f"Stopped after {num_iter} iterations.",
            )
