...
class EarlyStopper(Generic[TState]):
    ...
    def step(self, score: float, state: TState | None = None) -> bool:
        ...
        if score < self.min_score:
            self._steps_best_state = self.step_counter
    ...
    def best_state(self) -> TState:
        if self._best_state is None:
            raise RuntimeError("Best state not stored.")
        return self._best_state

    def steps_best_state(self) -> int:
        if self._best_state is None:
            raise RuntimeError("Best state not stored.")
        return self._steps_best_state
