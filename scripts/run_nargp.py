...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Run NARGP experiments.")
    parser.add_argument(
...
class NarGPKernel(torch.nn.Module):
    """Scalable HF climate kernel in NarGP style using SKI."""
    ...
    def __init__(
        ...
        k2 = GridInterpolationKernel(
            base_k2, grid_size=grid_size, grid_bounds=[bounds], active_dims=[0]
        )
        ...
        self.covar_module = ScaleKernel(signal_kernel)
...
def get_multifidelity_scores(
    n_ensemble: int, locs_pkl: dict, obs_pkl: dict
    ...
    niters = 1000
    train_lf = obs_train_lf[:n_ensemble, :]
    ...
    model_lf = GPMatern(locs_lf, train_lf[0], likelihood_lf).double()
    model_lf.train()
    likelihood_lf.train()
    ...
    with torch.no_grad():
        model_lf.eval()
        likelihood_lf.eval()
        y_hat_lf = likelihood_lf(model_lf(locs_mf)).loc

    x_mf = torch.hstack((y_hat_lf.reshape(100, 1), locs_mf))
    train_mf = obs_train_mf[:n_ensemble, :]
    ...
    train_hf = obs_train_hf[:n_ensemble, :]
    ...
    with torch.no_grad():
        model_lf.eval()
        likelihood_lf.eval()
        posterior = likelihood_lf(model_lf(locs_hf))
        ...
        for i in range(num_samples):
            ...
            tmp_m[i, :] = model_mf(x_hf).loc.flatten()
    ...
    with torch.no_grad():
        ...
        ls_lf = -model_lf_at_lf.log_prob(test_lf).detach().cpu()
    with torch.no_grad():
        model_mf.train()
...
def get_climate_scores(n: int) -> dict[str, np.ndarray | list[int]]:
    ...
    a_norm = (y_hat_lf - y_hat_lf.min()) / (y_hat_lf.max() - y_hat_lf.min()).clamp_min(
        1e-8
    )
    x_full = torch.cat([a_norm.view(-1, 1), locs_rcm], dim=1).contiguous()

    hf_cfg = HFConfig()
    torch.manual_seed(0)
    n_rcm = x_full.shape[0]
    m_induce = min(hf_cfg.m_induce, n_rcm)
    idx_z = torch.randperm(n_rcm, device=device)[:m_induce]
    Z = x_full[idx_z].detach()

    raw_ell = nn.Parameter(
        softplus_inv(torch.full((3,), 0.25, device=device, dtype=dtype))
    )
...
def main() -> None:
    ...
    for n in parse_ns(args.ns, args.experiment):
        ...
        save_rows(
            rows, args.output_dir, "nargp", args.experiment, args.include_all_logscores
        )
        print("mean log score")
