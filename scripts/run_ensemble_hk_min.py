...
def get_logscore(n, data_pkl, data_pkl_min):
    ...
    obs_lf = data_pkl_min["obs_lf"].to(torch.float64)
    obs_mf = data_pkl_min["obs_mf"].to(torch.float64)
    obs_hf = data_pkl_min["obs_hf"].to(torch.float64)
...
data_min = Path("../tests/data/data_mf_min.pkl")
