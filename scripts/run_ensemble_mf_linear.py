...
def get_logscore(n, data_pkl):
    ...
    tm.fit(nsteps, 0.1, test_data=tm.data, optimizer=opt, scheduler=sched)

    print("Evaluating log-scores")
    # leaving out some locations so that we can compare fairly with
    # the Matérn model that cannot multi-fidelity naturally
    left_out_hf = []
    for j in range(10):
        for i in range(10):
...
for n in ns:
    ...
    df.to_csv("./results/logscores_mf_linear.csv", index=False)
