...
class TFVAE(nn.Module):
    ...
    def forward(self, y_s, y_m, y_h):
        ...
        return (s_rec, m_rec, h_rec, s_mu, s_lv, m_mu, m_lv, h_mu, h_lv)
...
def compute_log_likelihood_tri(
    model, y_s, y_m, y_h, num_samples=5_000, device=None  # vectors: 25 / 100 / 900
):
    """
    ...
    with torch.no_grad():
        ...
        log_q_z = (
            diag_normal_log_prob(z_s, s_mu, s_lv)
            + diag_normal_log_prob(z_m, m_mu, m_lv)
            + diag_normal_log_prob(z_h, h_mu, h_lv)
        )  # (N,)
        ...
        log_p_y_given_z = (
            -0.5 * mse_s + const_s + -0.5 * mse_m + const_m + -0.5 * mse_h + const_h
        )  # (N,)
...
def get_logscore(n, data_pkl_min):
...
for n in ns:
    ...
    print(n)
    sc = get_logscore(n, data_pkl_min)
    n_list.append(n)
    ...
    df.to_csv("./results/logscores_VAE_min.csv", index=False)
