...
import math
from torch.distributions import Normal, kl_divergence   # only Normal is used below
...
class JointDecoder(nn.Module):
    ...
    def __init__(self, lf_latent=16, hf_latent=256):
        ...
        self.lf_head = nn.Sequential(
            ...
            nn.Upsample(size=(24,14), mode='bilinear',
                        align_corners=False)                       # 16×16→24×14
        )
        ...
        self.hf_head = nn.Sequential(
            ...
            nn.Upsample(size=(280,280), mode='bilinear',
                        align_corners=False)                       # 256→280
        )
...
class BFVAE(nn.Module):
    ...
    def forward(self, y_lf, y_hf):
        ...
        return (lf_rec, hf_rec,
                lf_mu, lf_lv,
                hf_mu, hf_lv)

    # pixel-wise MSE (unit variance likelihood) + KL
    def loss_fn(self, lf_rec, hf_rec,
                      lf_mu, lf_lv, hf_mu, hf_lv,
                      y_lf, y_hf, kl_weight=0.1):
...
class BiFiDataset(Dataset):
    def __init__(self, lf_vectors, hf_vectors,
                 lf_shape=(24, 14), hf_shape=(280, 280)):
...
def log_score(model, y_lf_vec, y_hf_vec, N=5_000, device=None):
    ...
    # expand to (N, z_dim)
    μ_lf = μ_lf.expand(N, -1);  lv_lf = lv_lf.expand(N, -1)
    μ_hf = μ_hf.expand(N, -1);  lv_hf = lv_hf.expand(N, -1)
    ...
    log_pz = (-0.5*(z_lf**2).sum(1) - 0.5*z_ldim*math.log(2*math.pi) +
              -0.5*(z_hf**2).sum(1) - 0.5*z_hdim*math.log(2*math.pi))
