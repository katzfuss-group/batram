...
from gpytorch.kernels import GridInterpolationKernel, MaternKernel, ScaleKernel, ProductKernel, AdditiveKernel
...
class NarGP(ApproximateGP):
    def __init__(self, train_x):
        ...
        k1 = GridInterpolationKernel(base_k1, grid_size=GRID,
                                     grid_bounds=[(-0.2, 1.2), (-0.2, 1.2)],
                                     active_dims=[1, 2])
        k2 = GridInterpolationKernel(base_k2, grid_size=GRID,
                                     grid_bounds=[(-0.2, 1.2)],
                                     active_dims=[0])
        k3 = GridInterpolationKernel(base_k3, grid_size=GRID,
                                     grid_bounds=[(-0.2, 1.2), (-0.2, 1.2)],
                                     active_dims=[1, 2])
...
def make_bounds(x, pad=0.05):             # pad = 5 % of range
    ...
    return [(l.item(), u.item()) for l, u in zip(lower, upper)]
...
def get_logscore(n):
    ...
    x_hf = torch.hstack((y_hat_lf.reshape(78400,1), locs_rcm))
    ...
    # Build inputs
    a_norm  = (y_hat_lf - y_hat_lf.min()) / (y_hat_lf.max() - y_hat_lf.min())  # shape [N]
    ...
    bounds_all = make_bounds(x_full, pad=0.15)          # list of 3 tuples
    a_bounds      = [bounds_all[0]]                     # dim-0
    latlon_bounds = bounds_all[1:]                      # dims-1&2 (2 tuples)
